...
export class PanelManager {
  ...
  async createPanel(): Promise<boolean> {
    ...
    // First, check if there's an existing Ollama Code group
    const existingGroup = this.findExistingQwenCodeGroup();

    if (existingGroup) {
      // If Ollama Code webview already exists in a locked group, create the new panel in that same group
      console.log(
        '[PanelManager] Found existing Ollama Code group, creating panel in same group',
      );
      ...
      // If no existing Ollama Code group, create a new group to the right of the active editor group
      try {
    ...
    for (const group of vscode.window.tabGroups.all) {
      for (const tab of group.tabs) {
        ...
        if (
          ...
          // Found an existing Ollama Code tab
          console.log('[PanelManager] Found existing Ollama Code group:', {
