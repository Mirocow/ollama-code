...
import { OllamaAgentManager } from '../services/ollamaAgentManager.js';
...
export class WebViewProvider {
    ...
    constructor(context, extensionUri) {
        ...
        this.agentManager = new OllamaAgentManager();
        ...
        // Note: Tool call updates are handled in handleSessionUpdate within OllamaAgentManager
        ...
        this.agentManager.onPermissionRequest(async (request) => {
            ...
            if (this.isAutoMode()) {
                ...
                const pick = (substr) => options.find((o) => (o.optionId || '').toLowerCase().includes(substr))?.optionId;
                const pickByKind = (k) => options.find((o) => (o.kind || '').toLowerCase().includes(k))
                    ?.optionId;
                const optionId = pick('allow_once') ||
                    pickByKind('allow') ||
                    pick('proceed') ||
                    options[0]?.optionId ||
                    'allow_once';
                    ...
                    try {
                        ...
                        if (!isCancel) {
                            try {
                                ...
                                console.warn('[WebViewProvider] Failed to close diffs after allow (resolver):', err);
                            }
                            try {
                                void vscode.commands.executeCommand('qwen.diff.suppressBriefly');
                            }
                            catch (err) {
                                console.warn('[WebViewProvider] Failed to suppress diffs briefly:', err);
                            }
                        }
                    }
                };
                const handler = (message) => {
                    ...
                    if (isCancel) {
                        ...
                        try {
                            ...
                            console.warn('[WebViewProvider] Failed to close diffs after reject:', err);
                        }
                    ...
                    // If user allowed/proceeded, proactively close any open qwen-diff editors and suppress re-open briefly
                        ...
                        try {
                            ...
                            console.warn('[WebViewProvider] Failed to close diffs after allow:', err);
                        }
                        try {
                            void vscode.commands.executeCommand('qwen.diff.suppressBriefly');
                        }
    ...
    async show() {
        ...
        newPanel.webview.html = WebViewContent.generate(newPanel, this.extensionUri);
    ...
    async doInitializeAgentConnection(options) {
        ...
        const run = async () => {
            ...
            const bundledCliEntry = vscode.Uri.joinPath(this.extensionUri, 'dist', 'qwen-cli', 'cli.js').fsPath;
            try {
                console.log('[WebViewProvider] Connecting to agent...');
                // Pass the detected CLI path to ensure we use the correct installation
                const connectResult = await this.agentManager.connect(workingDir, bundledCliEntry, options);
                ...
                if (connectResult.requiresAuth && !autoAuthenticate) {
                    console.log('[WebViewProvider] Authentication required but auto-auth disabled, sending authState and returning');
                    this.sendMessageToWebView({
                        type: 'authState',
                ...
                vscode.window.showWarningMessage(`Failed to connect to Qwen CLI: ${_error}\nYou can still use the chat UI, but messages won't be sent to AI.`);
    ...
    async forceReLogin() {
        ...
        return vscode.window.withProgress({
            ...
            try {
                ...
                console.error('[WebViewProvider] Error stack:', _error instanceof Error ? _error.stack : 'N/A');
    ...
    async loadCurrentSessionMessages(options) {
        ...
        try {
                ...
                if (!autoAuthenticate) {
                    ...
                    try {
                        ...
                        if (requiresAuth && !autoAuthenticate) {
                            ...
                            vscode.window.showWarningMessage(`Failed to create ACP session: ${sessionError}. You may need to authenticate first.`);
                        }
                    }
                }
            }
            ...
            vscode.window.showErrorMessage(`Failed to load session messages: ${_error}`);
            await this.initializeEmptyConversation();
            return false;
        }
        return sessionReady;
    ...
    async restorePanel(panel) {
        ...
        try {
            ...
            console.warn('[WebViewProvider] Failed to reset restored panel title:', e);
        }
