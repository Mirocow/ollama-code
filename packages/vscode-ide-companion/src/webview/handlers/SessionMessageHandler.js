...
export class SessionMessageHandler extends BaseMessageHandler {
    ...
    async handle(message) {
        ...
        switch (message.type) {
            case 'sendMessage':
                await this.handleSendMessage(data?.text || '', data?.context, data?.fileContext);
                break;
            case 'newQwenSession':
     ...
     */
    async promptLoginOrOffline(message) {
        const selection = await vscode.window.showWarningMessage(message, 'Login Now', 'View Offline');
     ...
     */
    async handleSendMessage(text, context, fileContext) {
        ...
        if (!this.agentManager.currentSessionId) {
            try {
                ...
                if (errorMsg.includes('Authentication required') ||
                    ...
                    await this.promptLogin('Your login session has expired or is invalid. Please login again to continue using Qwen Code.');
                    return;
                }
    ...
    async handleNewQwenSession() {
        try {
            ...
            if (!this.agentManager.isConnected) {
                const proceeded = await this.promptLogin('You need to login before creating a new session.');
                if (!proceeded) {
                    return;
                }
            }
            ...
            if (errorMsg.includes('Authentication required') ||
                ...
                errorMsg.includes('No active ACP session')) {
                // Show a more user-friendly error message for expired sessions
                await this.promptLogin('Your login session has expired or is invalid. Please login again to create a new session.');
                // Send a specific error to the webview for better UI handling
                this.sendToWebView({
                    type: 'sessionExpired',
                    data: { message: 'Session expired. Please login again.' },
    ...
    async handleSwitchQwenSession(sessionId) {
        try {
            ...
            if (!this.agentManager.isConnected) {
                const choice = await this.promptLoginOrOffline('You are not logged in. Login now to fully restore this session, or view it offline.');
                if (choice === 'offline') {
                    ...
                    vscode.window.showInformationMessage('Showing cached session content. Login to interact with the AI.');
                    return;
                }
            ...
            try {
                ...
                if (errorMsg.includes('Authentication required') ||
                    ...
                    errorMsg.includes('No active ACP session')) {
                    // Show a more user-friendly error message for expired sessions
                    await this.promptLogin('Your login session has expired or is invalid. Please login again to switch sessions.');
                    // Send a specific error to the webview for better UI handling
                    this.sendToWebView({
                        type: 'sessionExpired',
                        data: { message: 'Session expired. Please login again.' },
                ...
                if (this.agentManager.isConnected) {
                    try {
                        const newAcpSessionId = await this.agentManager.createNewSession(workingDir);
                        ...
                        if (createErrorMsg.includes('Authentication required') ||
                    ...
                    vscode.window.showWarningMessage('Showing cached session content. Login to interact with the AI.');
                }
            }
        }
        catch (error) {
    ...
    async handleGetQwenSessions(cursor, size) {
        try {
            ...
            if (errorMsg.includes('Authentication required') ||
                ...
                errorMsg.includes('No active ACP session')) {
                // Show a more user-friendly error message for expired sessions
                await this.promptLogin('Your login session has expired or is invalid. Please login again to view sessions.');
                // Send a specific error to the webview for better UI handling
                this.sendToWebView({
                    type: 'sessionExpired',
                    data: { message: 'Session expired. Please login again.' },
    ...
    async handleSaveSession(tag) {
        try {
            ...
            // Try ACP save first
            try {
                const response = await this.agentManager.saveSessionViaAcp(this.currentConversationId, tag);
                this.sendToWebView({
                ...
                if (errorMsg.includes('Authentication required') ||
                    ...
                    errorMsg.includes('No active ACP session')) {
                    // Show a more user-friendly error message for expired sessions
                    await this.promptLogin('Your login session has expired or is invalid. Please login again to save sessions.');
                    // Send a specific error to the webview for better UI handling
                    this.sendToWebView({
                        type: 'sessionExpired',
                        data: { message: 'Session expired. Please login again.' },
        ...
        try {
            ...
            try {
                ...
                if (errorMsg.includes('Authentication required') ||
                    ...
                    errorMsg.includes('No active ACP session')) {
                    // Show a more user-friendly error message for expired sessions
                    await this.promptLogin('Your login session has expired or is invalid. Please login again to resume sessions.');
                    // Send a specific error to the webview for better UI handling
                    this.sendToWebView({
                        type: 'sessionExpired',
                        data: { message: 'Session expired. Please login again.' },
