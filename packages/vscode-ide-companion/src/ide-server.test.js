...
const getPortFromMock = (replaceMock) => {
    const port = vi
        .mocked(replaceMock)
        .mock.calls.find((call) => call[0] === 'OLLAMA_CODE_IDE_SERVER_PORT')?.[1];
    if (port === undefined) {
...
describe('IDEServer', () => {
    ...
    it('should set environment variables and workspace path on start with multiple folders', async () => {
        ...
        expect(replaceMock).toHaveBeenNthCalledWith(1, 'OLLAMA_CODE_IDE_SERVER_PORT', expect.any(String));
        const expectedWorkspacePaths = [
            '/test/workspace1',
            '/test/workspace2',
        ].join(path.delimiter);
        expect(replaceMock).toHaveBeenNthCalledWith(2, 'OLLAMA_CODE_IDE_WORKSPACE_PATH', expectedWorkspacePaths);
        const port = getPortFromMock(replaceMock);
        const expectedLockFile = path.join('/home/test', '.ollama', 'ide', `${port}.lock`);
    ...
    it('should set a single folder path', async () => {
        ...
        await ideServer.start(mockContext);
        const replaceMock = mockContext.environmentVariableCollection.replace;
        expect(replaceMock).toHaveBeenCalledWith('OLLAMA_CODE_IDE_WORKSPACE_PATH', '/foo/bar');
        const port = getPortFromMock(replaceMock);
        const expectedLockFile = path.join('/home/test', '.ollama', 'ide', `${port}.lock`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
    ...
    it('should set an empty string if no folders are open', async () => {
        ...
        await ideServer.start(mockContext);
        const replaceMock = mockContext.environmentVariableCollection.replace;
        expect(replaceMock).toHaveBeenCalledWith('OLLAMA_CODE_IDE_WORKSPACE_PATH', '');
        const port = getPortFromMock(replaceMock);
        const expectedLockFile = path.join('/home/test', '.ollama', 'ide', `${port}.lock`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
    ...
    it('should update the path when workspace folders change', async () => {
        ...
        expect(replaceMock).toHaveBeenCalledWith('OLLAMA_CODE_IDE_WORKSPACE_PATH', expectedWorkspacePaths);
        const port = getPortFromMock(replaceMock);
        const expectedLockFile = path.join('/home/test', '.ollama', 'ide', `${port}.lock`);
        const expectedContent = JSON.stringify({
            port: parseInt(port, 10),
        ...
        await ideServer.syncEnvVars();
        expect(replaceMock).toHaveBeenCalledWith('OLLAMA_CODE_IDE_WORKSPACE_PATH', '/baz/qux');
