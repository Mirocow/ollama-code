...
describe('AcpSessionManager', () => {
    ...
    describe('setModel', () => {
        it('sends session/set_model request with correct parameters', async () => {
            ...
            const responsePromise = sessionManager.setModel('qwen3-coder-plus', mockChild, pendingRequests, nextRequestId);
        ...
        it('throws error when no active session', async () => {
            await expect(sessionManager.setModel('qwen3-coder-plus', mockChild, pendingRequests, nextRequestId)).rejects.toThrow('No active ACP session');
        });
        it('increments request ID for each call', async () => {
            // @ts-expect-error - accessing private property for testing
            sessionManager.sessionId = 'test-session-id';
            await sessionManager.setModel('model-1', mockChild, pendingRequests, nextRequestId);
            await sessionManager.setModel('model-2', mockChild, pendingRequests, nextRequestId);
    ...
    describe('setMode', () => {
        it('sends session/set_mode request with correct parameters', async () => {
            // @ts-expect-error - accessing private property for testing
            sessionManager.sessionId = 'test-session-id';
            const responsePromise = sessionManager.setMode('auto-edit', mockChild, pendingRequests, nextRequestId);
            const response = await responsePromise;
            expect(writtenMessages.length).toBe(1);
            const sentMessage = JSON.parse(writtenMessages[0].trim());
        ...
        it('throws error when no active session', async () => {
            await expect(sessionManager.setMode('default', mockChild, pendingRequests, nextRequestId)).rejects.toThrow('No active ACP session');
        });
    });
});
//# sourceMappingURL=acpSessionManager.test.js.map