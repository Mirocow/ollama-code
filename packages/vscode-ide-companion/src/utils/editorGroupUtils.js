...
export function findLeftGroupOfChatWebview() {
    try {
        ...
        const webviewGroup = groups.find((group) => group.tabs.some((tab) => {
            const input = tab.input;
            const isWebviewInput = (inp) => !!inp && typeof inp === 'object' && 'viewType' in inp;
            return (isWebviewInput(input) &&
 ...
 */
function waitForTabGroupsCondition(condition, timeout = 2000) {
    if (condition()) {
 ...
 */
export async function ensureLeftGroupOfChatWebview() {
    ...
    const groupCreated = await waitForTabGroupsCondition(() => vscode.window.tabGroups.all.length > initialGroupCount, 1000);
    if (!groupCreated) {
