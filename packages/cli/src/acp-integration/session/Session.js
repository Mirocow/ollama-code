 ...
 */
import { ApprovalMode, convertToFunctionResponse, createDebugLogger, DiscoveredMCPTool, ToolConfirmationOutcome, logToolCall, logUserPrompt, getErrorStatus, TaskTool, UserPromptEvent, TodoWriteTool, ExitPlanModeTool, readManyFiles, } from '@ollama-code/ollama-code-core';
import * as acp from '../acp.js';
import { z } from 'zod';
import { normalizePartList } from '../../utils/nonInteractiveHelpers.js';
import { handleSlashCommand, getAvailableCommands, } from '../../nonInteractiveCliCommands.js';
import { isSlashCommand } from '../../ui/utils/commandUtils.js';
import { formatAcpModelId, parseAcpModelOption, } from '../../utils/acpModelUtils.js';
...
export class Session {
    ...
    async prompt(params) {
        ...
        while (nextMessage !== null) {
            ...
            try {
                const responseStream = chat.sendMessageStream(this.config.getModel(), {
    ...
    async setModel(params) {
        ...
        if (!selectedAuthType) {
            throw acp.RequestError.invalidParams(`authType cannot be determined for modelId "${parsed.modelId}"`);
        }
    ...
    async runTool(abortSignal, promptId, fc) {
        ...
        try {
            ...
            if (isPlanMode && !isExitPlanModeTool && confirmationDetails) {
                ...
                return errorResponse(new Error(`Plan mode is active. The tool "${fc.name}" cannot be executed because it modifies the system. ` +
