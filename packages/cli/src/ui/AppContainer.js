...
import { IdeClient, ideContextStore, createDebugLogger, getErrorMessage, getAllOllamaMdFilenames, ShellExecutionService, Storage, } from '@ollama-code/ollama-code-core';
...
export const AppContainer = (props) => {
    ...
    const { setAuthState, authError, onAuthError, isAuthDialogOpen, isAuthenticating, pendingAuthType, qwenAuthState, handleAuthSelect, handleCodingPlanSubmit, openAuthDialog, cancelAuthentication, } = useAuthCommand(settings, config, historyManager.addItem, refreshStatic);
    ...
    const performMemoryRefresh = useCallback(async () => {
        historyManager.addItem({
            ...
            text: 'Refreshing hierarchical memory (OLLAMA_CODE.md or other context files)...',
        }, Date.now());
        try {
            const { memoryContent, fileCount } = await loadHierarchicalGeminiMemory(process.cwd(), settings.merged.context?.loadFromIncludeDirectories
                ? config.getWorkspaceContext().getDirectories()
                : [], config.getFileService(), config.getExtensionContextFilePaths(), config.isTrustedFolder(), settings.merged.context?.importFormat || 'tree');
