    ...
    lines.forEach((line, index) => {
        ...
        if (codeFenceMatch) {
            ...
            if (tableHeaders.length > 0 && tableRows.length > 0) {
                addContentBlock(_jsx(RenderTable, { headers: tableHeaders, rows: tableRows, contentWidth: contentWidth }, `table-${contentBlocks.length}`));
            }
            inTable = false;
            tableRows = [];
            tableHeaders = [];
...
const RenderTableInternal = ({ headers, rows, contentWidth, }) => (_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: contentWidth }));
