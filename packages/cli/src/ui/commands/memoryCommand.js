 ...
 */
import { getErrorMessage, getCurrentOllamaMdFilename, loadServerHierarchicalMemory, OLLAMA_DIR, } from '@ollama-code/ollama-code-core';
            ...
            subCommands: [
                {
                    ...
                    action: async (context) => {
                        try {
                            ...
                            const projectMemoryPath = path.join(workingDir, getCurrentOllamaMdFilename());
                ...
                },
                {
                    ...
                    action: async (context) => {
                        try {
                            const globalMemoryPath = path.join(os.homedir(), OLLAMA_DIR, getCurrentOllamaMdFilename());
                ...
                try {
                    ...
                    if (config) {
                        const { memoryContent, fileCount } = await loadServerHierarchicalMemory(config.getWorkingDir(), config.shouldLoadMemoryFromIncludeDirectories()
                            ...
                            : [], config.getFileService(), config.getExtensionContextFilePaths(), config.getFolderTrust(), context.services.settings.merged.context?.importFormat ||
                            'tree');
