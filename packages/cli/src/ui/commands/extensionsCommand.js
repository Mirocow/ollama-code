...
import { ExtensionManager, parseInstallSource, } from '@ollama-code/ollama-code-core';
...
async function updateAction(context, args) {
    ...
    try {
        ...
        await extensionManager.checkForAllExtensionUpdates((extensionName, state) => context.ui.dispatchExtensionStateUpdate({
            type: 'SET_STATE',
        ...
        if (all) {
            updateInfos = await extensionManager.updateAllUpdatableExtensions(context.ui.extensionsUpdateState, (extensionName, state) => context.ui.dispatchExtensionStateUpdate({
                type: 'SET_STATE',
                payload: { name: extensionName, state },
            }));
        }
            ...
            for (const name of names) {
                ...
                const updateInfo = await extensionManager.updateExtension(extension, context.ui.extensionsUpdateState.get(extension.name)?.status ??
                    ExtensionUpdateState.UNKNOWN, (extensionName, state) => context.ui.dispatchExtensionStateUpdate({
                    type: 'SET_STATE',
                    payload: { name: extensionName, state },
                }));
                if (updateInfo)
...
function getEnableDisableContext(context, argumentsString) {
    ...
    if (name === '' ||
        ...
        context.ui.addItem({
            type: MessageType.ERROR,
            text: t('Usage: /extensions {{command}} <extension> [--scope=<user|workspace>]', {
    ...
    switch (parts[2].toLowerCase()) {
        ...
        default:
            context.ui.addItem({
                type: MessageType.ERROR,
                text: t('Unsupported scope "{{scope}}", should be one of "user" or "workspace"', {
                    scope: parts[2],
                }),
            }, Date.now());
            return null;
...
async function detailAction(context, args) {
    const extensionManager = context.services.config?.getExtensionManager();
    if (!(extensionManager instanceof ExtensionManager)) {
        debugLogger.error(`Cannot ${context.invocation?.name} extensions in this environment`);
        return;
    ...
    context.ui.addItem({
        type: MessageType.INFO,
        text: extensionToOutputString(extension, extensionManager, process.cwd(), true),
    }, Date.now());
}
