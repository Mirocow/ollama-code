...
import { KittySequenceOverflowEvent, logKittySequenceOverflow, createDebugLogger, } from '@ollama-code/ollama-code-core';
        ...
        const handleKeypress = async (_, key) => {
            ...
            if ((key.ctrl && key.name === 'c') ||
                ...
                if (kittySequenceBuffer && debugKeystrokeLogging) {
                    debugLogger.debug('[DEBUG] Kitty buffer cleared on Ctrl+C:', kittySequenceBuffer);
                }
                kittySequenceBuffer = '';
            ...
            if (kittyProtocolEnabled) {
                if (kittySequenceBuffer ||
                    ...
                    if (debugKeystrokeLogging) {
                        debugLogger.debug('[DEBUG] Kitty buffer accumulating:', kittySequenceBuffer);
                    }
                    ...
                    while (kittySequenceBuffer) {
                        ...
                        if (!parsed) {
                            ...
                            if (nextStart > 0) {
                                if (debugKeystrokeLogging) {
                                    debugLogger.debug('[DEBUG] Skipping incomplete/invalid CSI prefix:', kittySequenceBuffer.slice(0, nextStart));
                                }
                    ...
                    if (kittySequenceBuffer.length > MAX_KITTY_SEQUENCE_LENGTH) {
                        if (debugKeystrokeLogging) {
                            debugLogger.debug('[DEBUG] Kitty buffer overflow, clearing:', kittySequenceBuffer);
                        }
                        if (config) {
