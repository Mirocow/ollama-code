...
export function SuggestionsDisplay({ suggestions, activeIndex, isLoading, width, scrollOffset, userInput, mode, expandedIndex, }) {
    if (isLoading) {
        return (_jsx(Box, { width: width, children: _jsx(Text, { color: "gray", children: "Loading suggestions..." }) }));
    }
    ...
    return (_jsxs(Box, { flexDirection: "column", width: width, children: [scrollOffset > 0 && _jsx(Text, { color: theme.text.primary, children: "\u25B2" }), visibleSuggestions.map((suggestion, index) => {
                ...
                return (_jsxs(Box, { flexDirection: "row", children: [_jsx(Box, { ...(mode === 'slash'
                                ...
                                : { flexShrink: 1 }), children: _jsxs(Box, { children: [labelElement, suggestion.commandKind === CommandKind.MCP_PROMPT && (_jsx(Text, { color: textColor, children: " [MCP]" }))] }) }), suggestion.description && (_jsx(Box, { flexGrow: 1, paddingLeft: 2, children: _jsx(Text, { color: textColor, wrap: "truncate", children: suggestion.description }) })), isActive && isLong && (_jsx(Box, { children: _jsx(Text, { color: Colors.Gray, children: isExpanded ? ' ← ' : ' → ' }) }))] }, `${suggestion.value}-${originalIndex}`));
