...
export const DEFAULT_THEME = OllamaDark;
class ThemeManager {
    availableThemes;
    activeTheme;
    ...
    getAvailableThemes() {
        ...
        const ollamaThemes = builtInThemes.filter((theme) => theme.name === OllamaLight.name || theme.name === OllamaDark.name);
        const otherBuiltInThemes = builtInThemes.filter((theme) => theme.name !== OllamaLight.name && theme.name !== OllamaDark.name);
