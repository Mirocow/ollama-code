...
import { getProjectSummaryInfo, } from '@ollama-code/ollama-code-core';
...
export function useWelcomeBack(config, submitQuery, buffer, settings) {
    ...
    const handleWelcomeBackSelection = useCallback((choice) => {
        ...
        if (choice === 'continue' && welcomeBackInfo?.content) {
            ...
            const contextMessage = `@.ollama-code/PROJECT_SUMMARY.md, Based on our previous conversation,Let's continue?`;
