    ...
    return async () => {
        ...
        if (currentMemory && currentMemory.trim().length > 0) {
            ...
            addMessage({
                type: MessageType.INFO,
                content: fileCount > 0
                    ? 'Hierarchical memory (OLLAMA_CODE.md or other context files) is loaded but content is empty.'
                    : 'No hierarchical memory (OLLAMA_CODE.md or other context files) is currently loaded.',
                timestamp: new Date(),
            });
        }
    };
