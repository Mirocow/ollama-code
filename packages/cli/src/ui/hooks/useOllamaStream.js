...
import { OllamaEventType as ServerOllamaEventType, createDebugLogger, getErrorMessage, isNodeError, MessageSenderType, logUserPrompt, GitService, UnauthorizedError, UserPromptEvent, logConversationFinishedEvent, ConversationFinishedEvent, ApprovalMode, parseAndFormatApiError, promptIdContext, ToolConfirmationOutcome, logApiCancel, ApiCancelEvent, } from '@ollama-code/ollama-code-core';
...
export const useOllamaStream = (ollamaClient, history, addItem, config, settings, onDebugMessage, handleSlashCommand, shellModeActive, getPreferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, onEditorClose, onCancelSubmit, visionModelPreviewEnabled, setShellInputFocused, terminalWidth, terminalHeight, onVisionSwitchRequired) => {
    ...
    const [pendingRetryErrorItem, setPendingRetryErrorItem] = useState(null);
    ...
    const { handleShellCommand, activeShellPtyId } = useShellCommandProcessor(addItem, setPendingHistoryItem, onExec, onDebugMessage, config, ollamaClient, setShellInputFocused, terminalWidth, terminalHeight);
    ...
    const prepareQueryForOllama = useCallback(async (query, userMessageTimestamp, abortSignal, prompt_id) => {
        if (turnCancelledRef.current) {
        ...
        let localQueryToSendToOllama = null;
        ...
        if (localQueryToSendToOllama === null) {
            onDebugMessage('Query processing resulted in null, not sending to Ollama.');
            return { queryToSend: null, shouldProceed: false };
        }
    ...
    const handleContentEvent = useCallback((eventValue, currentOllamaMessageBuffer, userMessageTimestamp) => {
        if (turnCancelledRef.current) {
            // Prevents additional output after a user initiated cancel.
            return '';
        }
        let newOllamaMessageBuffer = currentOllamaMessageBuffer + eventValue;
        if (pendingHistoryItemRef.current?.type !== 'ollama' &&
    ...
    const handleThoughtEvent = useCallback((eventValue, currentThoughtBuffer, userMessageTimestamp) => {
        ...
        const isPendingThought = pendingType === 'ollama_thought' ||
        ...
        const nextPendingType = isPendingThought && pendingType === 'ollama_thought_content'
    ...
    const processOllamaStreamEvents = useCallback(async (stream, userMessageTimestamp, signal) => {
        ...
        for await (const event of stream) {
            switch (event.type) {
                ...
                case ServerOllamaEventType.Content:
                    ollamaMessageBuffer = handleContentEvent(event.value, ollamaMessageBuffer, userMessageTimestamp);
                    break;
    ...
    const submitQuery = useCallback(async (query, options, prompt_id) => {
        ...
        return promptIdContext.run(prompt_id, async () => {
            const { queryToSend, shouldProceed } = await prepareQueryForOllama(query, userMessageTimestamp, abortSignal, prompt_id);
            ...
            setInitError(null);
            try {
                const stream = ollamaClient.sendMessageStream(finalQueryToSend, abortSignal, prompt_id, options);
                const processingStatus = await processOllamaStreamEvents(stream, userMessageTimestamp, abortSignal);
...
//# sourceMappingURL=useOllamaStream.js.map