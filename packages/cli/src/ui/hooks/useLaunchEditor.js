...
import { editorCommands, commandExists as coreCommandExists, } from '@ollama-code/ollama-code-core';
...
function getExecutableCommand(editorType) {
    ...
    if (!availableCommand) {
        throw new Error(`No available editor command found for ${editorType}. ` +
            `Tried: ${commands.join(', ')}. ` +
