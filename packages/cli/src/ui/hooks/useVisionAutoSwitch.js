...
import { AuthType, ApprovalMode, } from '@ollama-code/ollama-code-core';
...
import { isSupportedImageMimeType, getUnsupportedImageFormatWarning, } from '@ollama-code/ollama-code-core';
/**
...
export function useVisionAutoSwitch(config, addItem, visionModelPreviewEnabled = true, onVisionSwitchRequired) {
    ...
    const handleVisionSwitch = useCallback(async (query, userMessageTimestamp, isContinuation) => {
        ...
        if (!shouldOfferVisionSwitch(query, contentGeneratorConfig?.authType ?? AuthType.USE_OLLAMA, config.getModel(), visionModelPreviewEnabled)) {
            return { shouldProceed: true };
        }
