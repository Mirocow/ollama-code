...
import { createDebugLogger, logUserFeedback, UserFeedbackEvent, isNodeError, AuthType, } from '@ollama-code/ollama-code-core';
 ...
 */
const lastMessageIsAIResponse = (history) => history.length > 0 && history[history.length - 1].type === MessageType.OLLAMA;
/**
    ...
    useEffect(() => {
        const checkAndShowFeedback = () => {
            if (streamingState === StreamingState.Idle && history.length > 0) {
                ...
                if (config.getAuthType() !== AuthType.USE_OLLAMA ||
