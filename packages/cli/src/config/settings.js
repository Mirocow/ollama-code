...
import { FatalConfigError, OLLAMA_DIR, getErrorMessage, Storage, } from '@ollama-code/ollama-code-core';
...
const INVERTED_BOOLEAN_MIGRATIONS = {
...
export function getSystemSettingsPath() {
    if (process.env['OLLAMA_CODE_SYSTEM_SETTINGS_PATH']) {
...
export function getSystemDefaultsPath() {
    if (process.env['OLLAMA_CODE_SYSTEM_DEFAULTS_PATH']) {
        return process.env['OLLAMA_CODE_SYSTEM_DEFAULTS_PATH'];
    }
...
const KNOWN_V2_CONTAINERS = new Set([
    ...
    ...Object.values(INVERTED_BOOLEAN_MIGRATIONS).map((path) => path.split('.')[0]),
]);
export function needsMigration(settings) {
    ...
    const hasInvertedBooleanKeys = Object.keys(INVERTED_BOOLEAN_MIGRATIONS).some((v1Key) => v1Key in settings);
 ...
 */
function migrateV1ToV3(flatSettings) {
...
const V3_TO_V1_INVERTED_MAP = Object.fromEntries(Object.entries(INVERTED_BOOLEAN_MIGRATIONS).map(([v1Key, v3Path]) => [
    v3Path,
    v1Key,
]));
function getSettingsFileKeyWarnings(settings, settingsFilePath) {
    ...
    for (const key of Object.keys(settings)) {
        ...
        warnings.push(`Warning: Unknown setting '${key}' will be ignored in ${settingsFilePath}.`);
    }
    return warnings;
}
/**
...
function findEnvFile(settings, startDir) {
    ...
    const userLevelPaths = new Set([
        ...
        path.normalize(path.join(homeDir, OLLAMA_DIR, '.env')),
    ]);
    ...
    while (true) {
        // Prefer gemini-specific .env under OLLAMA_DIR
        const geminiEnvPath = path.join(currentDir, OLLAMA_DIR, '.env');
        ...
        if (parentDir === currentDir || !parentDir) {
            ...
            const homeGeminiEnvPath = path.join(homeDir, OLLAMA_DIR, '.env');
                ...
                if (needsMigration(settingsObject)) {
                    ...
                    if (migratedSettings) {
                        if (MIGRATE_V2_OVERWRITE) {
                            try {
                                ...
                                writeStderrLine(`Error migrating settings file on disk: ${getErrorMessage(e)}`);
                            }
                        }
                        else {
                    ...
                    if (MIGRATE_V2_OVERWRITE) {
                        try {
                            ...
                            writeStderrLine(`Error adding version to settings file: ${getErrorMessage(e)}`);
                        }
                    }
                }
                ...
                if (v3Migrated) {
                    if (MIGRATE_V2_OVERWRITE) {
                        try {
                            ...
                            fs.writeFileSync(filePath, JSON.stringify(v3Migrated, null, 2), 'utf-8');
                        }
                        catch (e) {
                            writeStderrLine(`Error migrating settings file to V3: ${getErrorMessage(e)}`);
                        }
                    }
                    else {
                        migratedInMemorScopes.add(scope);
