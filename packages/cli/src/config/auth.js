...
export function validateAuthMethod(authMethod, config) {
    ...
    if (authMethod === AuthType.USE_OLLAMA) {
        ...
        const isRemoteInstance = baseUrl &&
            !baseUrl.includes('localhost') &&
            !baseUrl.includes('127.0.0.1');
        if (isRemoteInstance) {
            // Remote instance - check for API key
            const { hasKey, checkedEnvKey } = hasApiKeyForAuth(authMethod, settings.merged, config);
            if (!hasKey) {
                const envKeyHint = checkedEnvKey || 'OLLAMA_API_KEY';
                return t('Remote Ollama instance detected. Set the {{envKeyHint}} environment variable for authentication.', { envKeyHint });
            }
        }
        return null;
    }
