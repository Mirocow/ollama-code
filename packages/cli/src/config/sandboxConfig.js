 ...
 */
import { FatalSandboxError } from '@ollama-code/ollama-code-core';
...
function getSandboxCommand(sandbox) {
    ...
    const environmentConfiguredSandbox = process.env['OLLAMA_SANDBOX']?.toLowerCase().trim() ??
    ...
    if (typeof sandbox === 'string' && sandbox) {
        ...
        throw new FatalSandboxError(`Missing sandbox command '${sandbox}' (from OLLAMA_SANDBOX or GEMINI_SANDBOX)`);
    }
    ...
    if (sandbox === true) {
        throw new FatalSandboxError('OLLAMA_SANDBOX (or GEMINI_SANDBOX) is true but failed to determine command for sandbox; ' +
