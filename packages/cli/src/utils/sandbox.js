...
export async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = []) {
    ...
    if (process.env['BUILD_SANDBOX']) {
        if (!gcPath.includes('ollama-code/packages/')) {
            throw new FatalSandboxError('Cannot build sandbox using installed Ollama Code binary; ' +
    ...
    const isIntegrationTest = process.env['OLLAMA_CLI_INTEGRATION_TEST'] === 'true' ||
        process.env['GEMINI_CLI_INTEGRATION_TEST'] === 'true'; // Legacy support
    let containerName;
    if (isIntegrationTest) {
        containerName = `ollama-code-integration-test-${randomBytes(4).toString('hex')}`;
