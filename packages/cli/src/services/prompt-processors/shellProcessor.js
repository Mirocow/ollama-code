 ...
 */
import { ApprovalMode, checkCommandPermissions, doesToolInvocationMatch, escapeShellArg, getShellConfiguration, ShellExecutionService, flatMapTextParts, } from '@ollama-code/ollama-code-core';
...
export class ShellProcessor {
    ...
    async processString(prompt, context) {
        ...
        for (const injection of resolvedInjections) {
            ...
            const isAllowedBySettings = doesToolInvocationMatch('run_shell_command', invocation, allowedTools);
            if (!allAllowed) {
                if (isHardDenial) {
