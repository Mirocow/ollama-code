...
export function SettingsPanel() {
  ...
  const [activeSection, setActiveSection] = useState<'general' | 'model' | 'tools' | 'security'>('general');
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSettings({ ...settings, theme: e.target.value as SettingsConfig['theme'] })}
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Allow AI to use file, shell, and other tools</p>
                </div>
                <button
                  onClick={() => setSettings({ ...settings, enableTools: !settings.enableTools })}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <label className="block text-sm font-medium">Enable MCP</label>
                  <p className="text-xs text-muted-foreground">Allow Model Context Protocol servers</p>
                </div>
                <button
                  onClick={() => setSettings({ ...settings, enableMcp: !settings.enableMcp })}
                  className={`w-12 h-6 rounded-full transition-colors ${
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSettings({ ...settings, approvalMode: e.target.value as SettingsConfig['approvalMode'] })}
                  className="w-full px-3 py-2 bg-background border border-border rounded-md"
                >
                  <option value="manual">Manual - Ask for each tool call</option>
          ...
          {activeSection === 'security' && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-2">Trusted Folders</label>
                <p className="text-xs text-muted-foreground mb-2">Folders where tools can run without confirmation</p>
                <div className="space-y-2">
                    ...
                    <div key={index} className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setSettings({ ...settings, trustedFolders: newFolders });
                        }}
                        className="flex-1 px-3 py-2 bg-background border border-border rounded-md"
                      />
                      <button
                        onClick={() => {
                          setSettings({
                            ...settings,
                            trustedFolders: settings.trustedFolders.filter((_, i) => i !== index),
                          });
                        }}
                  ...
                  <button
                    onClick={() => setSettings({ ...settings, trustedFolders: [...settings.trustedFolders, ''] })}
