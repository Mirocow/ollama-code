...
function findSdkPackageRoot(): string | null {
  try {
    ...
    const packageJsonPath = require.resolve('@ollama-code/sdk/package.json');
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (fs.existsSync(cliPath)) {
        try {
          ...
          if (packageJson.name === '@ollama-code/sdk') {
            return dir;
          }
          if (!bestMatch) {
            bestMatch = dir;
...
export function findBundledCliPath(): string {
  ...
  const candidates = getBundledCliCandidatePaths();
  throw new Error(
    'Bundled Ollama Code CLI not found. The CLI should be included in the SDK package.\n' +
      'Searched locations:\n' +
