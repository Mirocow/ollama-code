...
A minimum experimental TypeScript SDK for programmatic access to Ollama Code.
...
Creates a new query session with the Ollama Code.
...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'llama3.2'`, `'codellama'`, `'deepseek-coder'`). Takes precedence over `OPENAI_MODEL` and `OLLAMA_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                       |
| `pathToOllamaExecutable` | `string`                                       | Auto-detected    | Path to the Ollama Code executable. Supports multiple formats: `'ollama-code'` (native binary from PATH), `'/path/to/ollama-code'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `OLLAMA_CODE_CLI_PATH` env var, `~/.volta/bin/ollama-code`, `~/.npm-global/bin/ollama-code`, `/usr/local/bin/ollama-code`, `~/.local/bin/ollama-code`, `~/node_modules/.bin/ollama-code`, `~/.yarn/bin/ollama-code`. |
...
| `env`                    | `Record<string, string>`                       | -                | Environment variables to pass to the Ollama Code process. Merged with the current process environment.                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `authType`               | `'openai' \| 'ollama-oauth'`                   | `'openai'`       | Authentication type for the AI service. Using `'ollama-oauth'` in SDK is not recommended as credentials are stored in `~/.ollama-code` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                     |
...
```typescript
const q = query({ prompt: 'Your prompt', options: {
  timeout: {
...
```typescript
import { query, type SDKUserMessage } from '@ollama-code/sdk';
...
```typescript
import { query, type CanUseTool } from '@ollama-code/sdk';
...
```typescript
import { z } from 'zod';
import { query, tool, createSdkMcpServer } from '@ollama-code/sdk';
...
```typescript
import { query, isAbortError } from '@ollama-code/sdk';
...
```typescript
import { AbortError, isAbortError } from '@ollama-code/sdk';
...
#### Ollama Code Installation Required

Version 0.1.0 requires [Ollama Code](https://github.com/ollama-code/ollama-code) **>= 0.4.0** to be installed separately and accessible in your PATH.

```bash
# Install Ollama Code globally
npm install -g ollama-code@^0.4.0
```

**Note**: From version **0.1.1** onwards, the CLI is bundled with the SDK, so no separate Ollama Code installation is needed.
