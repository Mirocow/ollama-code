...
| `model`                  | `string`                                       | -                | The AI model to use (e.g., `'qwen2.5-coder'`, `'qwen2.5-coder'`, `'qwen2.5-coder'`). Takes precedence over `OPENAI_MODEL` and `OLLAMA_MODEL` environment variables.                                                                                                                                                                                                                                                                                                                       |
| `pathToQwenExecutable`   | `string`                                       | Auto-detected    | Path to the Ollama Code executable. Supports multiple formats: `'qwen'` (native binary from PATH), `'/path/to/qwen'` (explicit path), `'/path/to/cli.js'` (Node.js bundle), `'node:/path/to/cli.js'` (force Node.js runtime), `'bun:/path/to/cli.js'` (force Bun runtime). If not provided, auto-detects from: `OLLAMA_CODE_CLI_PATH` env var, `~/.volta/bin/qwen`, `~/.npm-global/bin/qwen`, `/usr/local/bin/qwen`, `~/.local/bin/qwen`, `~/node_modules/.bin/qwen`, `~/.yarn/bin/qwen`. |
...
| `authType`               | `'openai' \| 'qwen-oauth'`                     | `'openai'`       | Authentication type for the AI service. Using `'qwen-oauth'` in SDK is not recommended as credentials are stored in `~/.ollama-code` and may need periodic refresh.                                                                                                                                                                                                                                                                                                                       |
...
await q.setModel('qwen2.5-coder');
...
Version 0.1.0 requires [Ollama Code](https://github.com/QwenLM/ollama-code) **>= 0.4.0** to be installed separately and accessible in your PATH.
