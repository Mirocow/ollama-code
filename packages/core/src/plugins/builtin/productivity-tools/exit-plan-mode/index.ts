...
Plans are saved to \`model_storage\` namespace \`plans\` with:
- Automatic TTL of 7 days
- Link to todos for progress tracking
- Optional save to knowledge base for semantic search

## Verification Options

- \`verification.autoVerify\`: Auto-check plan completion
- \`verification.requiredFiles\`: Files that must exist
- \`verification.testCommands\`: Tests to run
- \`verification.checkCommands\`: Build/lint commands

## Knowledge Base

Set \`saveToKnowledge=true\` to enable semantic search for this plan.
