...
const mcpToolsPlugin: PluginDefinition = {
  ...
  aliases: [
    { alias: 'mcp', canonicalName: 'mcp_client', description: 'Model Context Protocol' },
    { alias: 'model_context', canonicalName: 'mcp_client', description: 'Model Context Protocol client' },
  ],
  ...
  prompts: [
    {
      priority: 1,
      content: 'MCP (Model Context Protocol) enables external tool integration. MCP servers provide additional tools discovered at runtime. Check config for configured MCP servers.',
    },
    {
      priority: 2,
      content: 'MCP TOOLS: Dynamically discovered from configured servers. Each MCP server can provide multiple tools. Tools appear with server prefix (e.g., mcp_server_tool_name).',
    },
    {
      priority: 3,
      content: 'Configure MCP servers in settings. Supports stdio, HTTP, WebSocket transports. MCP enables unlimited extensibility with third-party tool servers.',
    },
  ],
