...
- Batch: Execute multiple operations in one call`,
  parametersJsonSchema: {
    ...
    properties: {
      operation: {
        type: 'string',
        enum: ['set', 'get', 'delete', 'list', 'append', 'merge', 'clear', 'exists', 'stats', 'batch', 'backup', 'restore'],
...
interface StorageParams {
  operation: 'set' | 'get' | 'delete' | 'list' | 'append' | 'merge' | 'clear' | 'exists' | 'stats' | 'batch' | 'backup' | 'restore';
  namespace: string;
  key?: string;
  value?: unknown;
  persistent?: boolean;
...
export async function cleanupExpiredEntries(): Promise<{ session: number; persistent: number }> {
  ...
  try {
    const globalStorageDir = path.join(Storage.getGlobalOllamaDir(), STORAGE_DIR);
    const projectStorageDir = path.join(await findProjectRoot(), '.ollama-code', STORAGE_DIR);
        ...
        try {
          ...
          for (const [key, rawEntry] of Object.entries(data)) {
            ...
            if (isExpired(entry)) {
              persistentCleaned++;
              hasExpired = true;
              debugLogger.info(`[TTL] Cleaned persistent entry: ${file.replace('.json', '')}:${key}`);
            } else {
              validData[key] = entry;
            }
          }
  ...
  if (sessionCleaned > 0 || persistentCleaned > 0) {
    debugLogger.info(`[TTL] Cleanup completed: ${sessionCleaned} session, ${persistentCleaned} persistent`);
  }
...
async function performSet(params: StorageParams): Promise<ToolResult> {
  ...
  if (useSession) {
    ...
    uiTelemetryService.recordStorageOperation(`session:${getSessionNamespaceKey(namespace)}:${key}`, JSON.stringify(value).length);
    ...
    uiTelemetryService.recordStorageOperation(`${scope}:${namespace}:${key}`, JSON.stringify(value).length);
...
async function performBackup(params: StorageParams): Promise<ToolResult> {
  ...
  try {
    ...
    if (sessionStorage.size > 0) {
      ...
      for (const [nsKey, nsData] of sessionStorage.entries()) {
        ...
        if (Object.keys(validEntries).length > 0) {
          backupData.namespaces['__session__'][nsKey] = { value: validEntries, metadata: createMetadata() } as StorageEntry;
        }
      }
    }
    ...
    const totalKeys = Object.values(backupData.namespaces)
...
async function performRestore(params: StorageParams): Promise<ToolResult> {
      ...
      const backupList = await Promise.all(
        backups.map(async (f) => {
          try {
            ...
            const keyCount = Object.values(data.namespaces || {})
              .reduce((sum: number, ns: unknown) => sum + Object.keys(ns as object).length, 0);
  ...
  try {
    ...
    for (const [namespace, data] of Object.entries(backupData.namespaces || {})) {
...
async function initializeStorageMetrics(): Promise<void> {
  try {
    ...
    try {
      const projectRoot = await findProjectRoot();
      const projectStorageDir = path.join(projectRoot, '.ollama-code', STORAGE_DIR);
    ...
    debugLogger.info(`[StorageTool] Initialized storage metrics: ${totalKeys} keys, ${totalSize} bytes`);
  } catch (error) {
    debugLogger.error('[StorageTool] Error initializing storage metrics:', error);
  }
}
...
const storageToolsPlugin: PluginDefinition = {
  ...
  aliases: [
    { alias: 'storage', canonicalName: 'model_storage', description: 'Universal storage for AI model' },
    { alias: 'model_storage', canonicalName: 'model_storage', description: 'Model storage' },
    { alias: 'store', canonicalName: 'model_storage', description: 'Store and retrieve data' },
    { alias: 'kv', canonicalName: 'model_storage', description: 'Key-value storage' },
    { alias: 'cache', canonicalName: 'model_storage', description: 'Cache storage' },
    { alias: 'roadmap', canonicalName: 'model_storage', description: 'Project roadmap storage' },
    { alias: 'persist', canonicalName: 'model_storage', description: 'Persistent storage' },
  ],
  ...
  prompts: [
    {
      priority: 1,
      content: 'Storage tool (model_storage) is universal key-value storage for AI models. Supports persistent and session storage, TTL, metadata, batch operations. Namespaces: roadmap, session, knowledge, context, learning, metrics.',
    },
    {
      priority: 2,
      content: 'STORAGE OPERATIONS: set/get/delete for single items, list for keys, append for arrays, merge for objects, batch for multiple ops. Use TTL for temporary data, tags for categorization.',
    },
    {
      priority: 3,
      content: 'NAMESPACE GUIDELINES: roadmap (project plans), session (temporary data), knowledge (learned patterns), context (current task), learning (corrections), metrics (statistics). Persistent by default except session/context.',
    },
  ],
  ...
  hooks: {
    onLoad: async (context) => {
      ...
      try {
        ...
        debugLogger.error('[StorageTool] Failed to create storage directory:', err);
      }
