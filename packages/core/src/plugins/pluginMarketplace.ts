...
export class PluginMarketplace {
  ...
  private cache: Map<string, { data: MarketplacePlugin; timestamp: number }> = new Map();
  ...
  async search(options: MarketplaceSearchOptions = {}): Promise<MarketplacePlugin[]> {
    ...
    try {
      ...
      if (keywords.length > 0) searchTerms.push(...keywords.map(k => `keywords:${k}`));
      ...
      // Transform results
      let plugins = data.objects.map(obj => this.transformRegistryPackage(obj));
  ...
  async getPlugin(pluginId: string): Promise<MarketplacePlugin | null> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const scopedResponse = await fetch(`${this.npmRegistry}/@ollama-code/${pluginId}`);
        if (!scopedResponse.ok) {
          return null;
        }
        const data = await scopedResponse.json();
        return this.transformPackageManifest(data);
      }
      
      const data = await response.json();
      const plugin = this.transformPackageManifest(data);
  ...
  async install(
  ...
  ): Promise<{ success: boolean; message: string; plugin?: MarketplacePlugin }> {
    const {
      version,
      global = true,
      force = false,
    ...
    debugLogger.info(`Installing plugin: ${pluginId}${version ? `@${version}` : ''}`);
    
    try {
      // Get plugin info
      const plugin = await this.getPlugin(pluginId);
      if (!plugin) {
        return { success: false, message: `Plugin "${pluginId}" not found in marketplace` };
      }
      ...
      return {
        ...
        plugin: { ...plugin, installed: true, installedVersion: plugin.version },
      };
    } catch (error) {
   ...
   * Uninstall a plugin
   */
  async uninstall(pluginId: string, options: { global?: boolean } = {}): Promise<{ success: boolean; message: string }> {
  ...
  async update(
    ...
    try {
      ...
      if (currentVersion === targetVersion) {
        return { success: true, message: `Plugin "${pluginId}" is already up to date`, updated: false };
      }
   ...
   * Update all installed plugins
   */
  async updateAll(options: PluginUpdateOptions = {}): Promise<Array<{ pluginId: string; success: boolean; message: string }>> {
  ...
  async getInstalledPlugins(): Promise<MarketplacePlugin[]> {
    ...
    const userPlugins = await this.scanPluginDirectory(this.userPluginsDir, 'user');
    plugins.push(...userPlugins);
    
    // Check project-level plugins
    if (this.projectPluginsDir) {
      const projectPlugins = await this.scanPluginDirectory(this.projectPluginsDir, 'project');
   ...
   */
  private async getInstalledPlugin(pluginId: string): Promise<MarketplacePlugin | null> {
    const installed = await this.getInstalledPlugins();
    return installed.find(p => p.id === pluginId || p.packageName === pluginId) || null;
  }
  ...
  private readInstalledPlugin(pluginPath: string): MarketplacePlugin | null {
    try {
      ...
      return {
        id: manifest?.metadata.id || packageJson.name.replace(/^ollama-code-plugin-/, ''),
   ...
   */
  private transformRegistryPackage(obj: RegistryResponse['objects'][0]): MarketplacePlugin {
    const pkg = obj.package;
    const id = pkg.name.replace(/^ollama-code-plugin-/, '').replace(/^@ollama-code\//, '');
   ...
   */
  private transformPackageManifest(data: Record<string, unknown>): MarketplacePlugin {
    ...
    const id = name.replace(/^ollama-code-plugin-/, '').replace(/^@ollama-code\//, '');
    ...
    return {
      ...
      description: (versionData['description'] as string) || (data['description'] as string) || '',
      version,
      author: (versionData['author'] as { name: string }) || (data['author'] as { name: string }),
      keywords: ((versionData['keywords'] || data['keywords']) as string[]) || [],
      ...
      repository: typeof versionData['repository'] === 'string' 
        ...
        : (versionData['repository'] as { url: string } | undefined)?.url || (data['repository'] as string),
  ...
  private applyFilters(
    ...
    if (options.trustLevel?.length) {
      filtered = filtered.filter(p => options.trustLevel!.includes(p.trustLevel));
    }
   ...
   */
  private async addInstallationStatus(plugins: MarketplacePlugin[]): Promise<MarketplacePlugin[]> {
    const installed = await this.getInstalledPlugins();
 ...
 */
export function createPluginMarketplace(projectRoot?: string): PluginMarketplace {
