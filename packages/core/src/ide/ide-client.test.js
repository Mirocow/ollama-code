...
describe('IdeClient', () => {
    ...
    describe('connect', () => {
        it('should connect using HTTP when port is provided in config file', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.ollama-code', 'ide', '8080.lock'), 'utf8');
    ...
    describe('getConnectionConfigFromFile', () => {
        it('should return config from the env port lock file if it exists', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.ollama-code', 'ide', '1234.lock'), 'utf8');
            delete process.env['OLLAMA_CODE_IDE_SERVER_PORT'];
        });
        ...
        it('should read legacy pid config when available', async () => {
            ...
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(config);
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'ollama-code-ide-server-12345.json'), 'utf8');
        });
        it('should fall back to legacy port file when pid file is missing', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'ollama-code-ide-server-12345.json'), 'utf8');
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'ollama-code-ide-server-2222.json'), 'utf8');
            delete process.env['OLLAMA_CODE_IDE_SERVER_PORT'];
        });
