...
export class IdeClient {
    ...
    async connect() {
        ...
        const workspacePath = this.connectionConfig?.workspacePath ??
            process.env['OLLAMA_CODE_IDE_WORKSPACE_PATH'];
    ...
    getPortFromEnv() {
        const port = process.env['OLLAMA_CODE_IDE_SERVER_PORT'];
        if (!port) {
            return undefined;
        }
        return port;
    ...
    getStdioConfigFromEnv() {
        const command = process.env['OLLAMA_CODE_IDE_SERVER_STDIO_COMMAND'];
        if (!command) {
            return undefined;
        }
        const argsStr = process.env['OLLAMA_CODE_IDE_SERVER_STDIO_ARGS'];
        let args = [];
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('OLLAMA_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
                debugLogger.error('Failed to parse OLLAMA_CODE_IDE_SERVER_STDIO_ARGS:', e);
            }
        }
        return { command, args };
    }
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `ollama-code-ide-server-${this.ideProcessInfo.pid}.json`);
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `ollama-code-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
