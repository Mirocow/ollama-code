...
export class IdeClient {
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. Ollama Code is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
    ...
    if (argsStr) {
      try {
        ...
        if (Array.isArray(parsedArgs)) {
          ...
          debugLogger.error(
            'OLLAMA_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
          );
        }
      } catch (e) {
        debugLogger.error(
          'Failed to parse OLLAMA_CODE_IDE_SERVER_STDIO_ARGS:',
          e,
        );
      }
    }
