...
function createTempFilesForModify(currentContent, proposedContent, file_path) {
    ...
    const diffDir = path.join(tempDir, 'ollama-code-tool-modify-diffs');
    ...
    const tempOldPath = path.join(diffDir, `ollama-code-modify-${fileName}-old-${timestamp}${ext}`);
    const tempNewPath = path.join(diffDir, `ollama-code-modify-${fileName}-new-${timestamp}${ext}`);
