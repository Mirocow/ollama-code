...
describe('ShellTool', () => {
    ...
    describe('build', () => {
        ...
        it('should throw an error for a directory within the user skills directory', () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.ollama-code/skills/my-skill',
                is_background: false,
        ...
        it('should throw an error for the user skills directory itself', () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.ollama-code/skills',
                is_background: false,
            })).toThrow('Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.');
        });
        it('should resolve directory path before checking user skills directory', () => {
            expect(() => shellTool.build({
                command: 'ls',
                directory: '/test/dir/.ollama-code/skills/../skills/my-skill',
                is_background: false,
            })).toThrow('Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.');
        });
    ...
    describe('execute', () => {
        ...
        it('should wrap command on linux and parse pgrep output', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir', expect.any(Function), expect.any(AbortSignal), false, {});
        ...
        it('should add ampersand to command when is_background is true and command does not end with &', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
        });
        ...
        it('should use the provided directory as cwd', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir/subdir', expect.any(Function), expect.any(AbortSignal), false, {});
        });
        ...
        it('should summarize output when configured', async () => {
            ...
            const result = await promise;
            expect(summarizer.summarizeToolOutput).toHaveBeenCalledWith(expect.any(String), mockConfig.getOllamaClient(), expect.any(AbortSignal), 1000);
        ...
        describe('addCoAuthorToGitCommit', () => {
            it('should add co-author to git commit with double quotes', async () => {
                ...
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Ollama-Coder <ollama-coder@local>'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
            });
 ...
 spanning multiple lines"`;
    ...
    describe('Windows background execution', () => {
        it('should clean up trailing ampersand on Windows for background tasks', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm start', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
        });
    });
    describe('timeout parameter', () => {
        ...
        it('should not create timeout signal for background execution', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.any(String), expect.any(String), expect.any(Function), mockAbortSignal, false, {});
        });
