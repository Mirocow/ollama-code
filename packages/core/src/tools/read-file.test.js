...
describe('ReadFileTool', () => {
    ...
    describe('execute', () => {
        ...
        describe('with .ollama-codeignore', () => {
            beforeEach(async () => {
                await fsp.writeFile(path.join(tempRootDir, '.ollama-codeignore'), ['foo.*', 'ignored/'].join('\n'));
            });
            it('should throw error if path is ignored by a .ollama-codeignore pattern', async () => {
                const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
                await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
                const params = {
                    absolute_path: ignoredFilePath,
                };
                const expectedError = `File path '${ignoredFilePath}' is ignored by .ollama-codeignore pattern(s).`;
