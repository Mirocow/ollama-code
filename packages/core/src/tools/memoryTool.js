...
const memoryToolSchemaData = {
    ...
    parametersJsonSchema: {
        ...
        properties: {
            ...
            scope: {
                type: 'string',
                description: 'Where to save the memory: "global" saves to user-level ~/.ollama-code/OLLAMA_CODE.md (shared across all projects), "project" saves to current project\'s OLLAMA_CODE.md (project-specific). If not specified, will prompt user to choose.',
                enum: ['global', 'project'],
            },
        },
        required: ['fact'],
...
let currentOllamaMdFilename = DEFAULT_CONTEXT_FILENAME;
export function setOllamaMdFilename(newFilename) {
...
Project: ${projectPath} (current project only)`;
        ...
        try {
            ...
            debugLogger.error(`[MemoryTool] Error executing save_memory for fact "${fact}" in ${scope}: ${errorMessage}`);
            return {
