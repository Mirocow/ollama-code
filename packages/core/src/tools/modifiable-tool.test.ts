...
describe('modifyWithEditor', () => {
  ...
  afterEach(async () => {
    ...
    const diffDir = path.join(os.tmpdir(), 'ollama-code-tool-modify-diffs');
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/ollama-code-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/ollama-code-modify-test-file-new-\d+\.txt$/);
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/ollama-code-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/ollama-code-modify-test-file-new-\d+$/);
