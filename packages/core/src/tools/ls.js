...
class LSToolInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        try {
            ...
            const { filteredPaths, gitIgnoredCount, ollamaCodeIgnoredCount } = fileDiscovery.filterFilesWithReport(relativePaths, {
                respectGitIgnore: this.params.file_filtering_options?.respect_git_ignore ??
                    this.config.getFileFilteringOptions().respectGitIgnore ??
                    DEFAULT_FILE_FILTERING_OPTIONS.respectGitIgnore,
                respectOllamaCodeIgnore: this.params.file_filtering_options?.respect_qwen_ignore ??
                    this.config.getFileFilteringOptions().respectOllamaCodeIgnore ??
            ...
            if (ollamaCodeIgnoredCount > 0) {
                ignoredMessages.push(`${ollamaCodeIgnoredCount} qwen-ignored`);
            }
        ...
        super(LSTool.Name, ToolDisplayNames.LS, 'Lists the names of files and subdirectories directly within a specified directory path. Can optionally ignore entries matching provided glob patterns.', Kind.Search, {
            properties: {
                ...
                file_filtering_options: {
                    description: 'Optional: Whether to respect ignore patterns from .gitignore or .ollama-codeignore',
                    type: 'object',
                    properties: {
                        respect_git_ignore: {
                        ...
                        respect_qwen_ignore: {
                            description: 'Optional: Whether to respect .ollama-codeignore patterns when listing files. Defaults to true.',
                            type: 'boolean',
                        },
                    },
                },
