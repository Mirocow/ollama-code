...
async function convertAgentFiles(agentsDir) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const systemPrompt = ollamaAgent['systemPrompt'] || body.trim();
            const newContent = `---
${newYaml}
---
 ...
 */
export function convertClaudeToOllamaConfig(claudeConfig) {
    ...
    if (claudeConfig.hooks) {
        debugLogger.warn(`[Claude Converter] Hooks are not yet supported in ${claudeConfig.name}`);
    }
...
export async function convertClaudePluginPackage(extensionDir, pluginName) {
    ...
    try {
        ...
        fs.writeFileSync(ollamaConfigPath, JSON.stringify(ollamaConfig, null, 2), 'utf-8');
        return {
            config: ollamaConfig,
            convertedDir: tmpDir,
        };
