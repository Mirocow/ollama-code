...
function createExtension({ extensionsDir = 'extensions-dir', name = 'my-extension', version = '1.0.0', addContextFile = false, contextFileName = undefined, mcpServers = {}, installMetadata = undefined, } = {}) {
    ...
    if (addContextFile) {
        fs.writeFileSync(path.join(extDir, 'OLLAMA_CODE.md'), 'context');
    }
    if (contextFileName) {
...
describe('extension tests', () => {
    ...
    beforeEach(() => {
        tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ollama-code-test-home-'));
        tempWorkspaceDir = fs.mkdtempSync(path.join(tempHomeDir, 'ollama-code-test-workspace-'));
    ...
    describe('loadExtension', () => {
        ...
        it('should load context file path when OLLAMA_CODE.md is present', async () => {
            createExtension({
                extensionsDir: userExtensionsDir,
                name: 'ext1',
                version: '1.0.0',
            ...
            expect(ext1?.contextFiles).toEqual([
                path.join(userExtensionsDir, 'ext1', 'OLLAMA_CODE.md'),
            ]);
        ...
        it('should use default OLLAMA_CODE.md when contextFileName is empty array', async () => {
            ...
            fs.writeFileSync(path.join(extDir, 'OLLAMA_CODE.md'), 'context content');
            const manager = createExtensionManager();
            await manager.refreshCache();
            const extensions = manager.getLoadedExtensions();
            expect(extensions).toHaveLength(1);
            ...
            expect(ext?.contextFiles).toEqual([
                path.join(userExtensionsDir, 'ext-empty-context', 'OLLAMA_CODE.md'),
            ]);
        });
