...
describe('FileDiscoveryService', () => {
    ...
    describe('initialization', () => {
        ...
        it('should load .ollama-codeignore patterns even when not in a git repo', async () => {
            await createTestFile('.ollama-codeignore', 'secrets.txt');
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldOllamaCodeIgnoreFile('secrets.txt')).toBe(true);
            expect(service.shouldOllamaCodeIgnoreFile('src/index.js')).toBe(false);
        });
    });
    describe('filterFiles', () => {
        beforeEach(async () => {
            await fs.mkdir(path.join(projectRoot, '.git'));
            await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
            await createTestFile('.ollama-codeignore', 'logs/');
        });
        ...
        it('should not filter files when respectGitIgnore is false', () => {
            ...
            const filtered = service.filterFiles(files, {
                ...
                respectOllamaCodeIgnore: true, // still respect this one
            });
            expect(filtered).toEqual(['src/index.ts', 'node_modules/package/index.js', '.git/config'].map((f) => path.join(projectRoot, f)));
        });
        it('should not filter files when respectOllamaCodeIgnore is false', () => {
            const files = [
                'src/index.ts',
                'node_modules/package/index.js',
                'logs/latest.log',
    ...
    describe('shouldGitIgnoreFile & shouldOllamaCodeIgnoreFile', () => {
        beforeEach(async () => {
            await fs.mkdir(path.join(projectRoot, '.git'));
            await createTestFile('.gitignore', 'node_modules/');
            await createTestFile('.ollama-codeignore', '*.log');
        });
        ...
        it('should return true for qwen-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldOllamaCodeIgnoreFile(path.join(projectRoot, 'debug.log'))).toBe(true);
        });
        it('should return false for non-qwen-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldOllamaCodeIgnoreFile(path.join(projectRoot, 'src/index.ts'))).toBe(false);
        });
    });
    describe('edge cases', () => {
