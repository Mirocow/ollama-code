...
export class OllamaLogger {
    ...
    constructor(config) {
        this.config = config;
        this.debugLogger = createDebugLogger('OLLAMA_LOGGER');
    ...
    enqueueLogEvent(event) {
        try {
            ...
            if (wasAtCapacity) {
                this.debugLogger.debug(`OllamaLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`);
            }
        }
        catch (error) {
            this.debugLogger.error('OllamaLogger: Failed to enqueue log event.', error);
        }
    }
    ...
    readSourceInfo() {
        try {
            const sourceJsonPath = path.join(os.homedir(), '.ollama', 'source.json');
    ...
    async flushToRum() {
        if (this.isFlushInProgress) {
            this.debugLogger.debug('OllamaLogger: Flush already in progress, marking pending flush.');
    ...
    requeueFailedEvents(eventsToSend) {
        ...
        if (eventsToSend.length > MAX_RETRY_EVENTS) {
            this.debugLogger.warn(`OllamaLogger: Dropping ${eventsToSend.length - MAX_RETRY_EVENTS} events due to retry queue limit. Total events: ${eventsToSend.length}, keeping: ${MAX_RETRY_EVENTS}`);
        }
        ...
        this.debugLogger.debug(`OllamaLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`);
    }
}
...
//# sourceMappingURL=ollama-logger.js.map