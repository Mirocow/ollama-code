...
describeIntegration('OllamaNativeClient Integration Tests', () => {
  ...
  describe('Model Operations', () => {
    ...
    it('should copy and delete a model', async () => {
      ...
      const copyStillExists = updatedModels.some((m) =>
        m.name.includes(COPY_MODEL_NAME),
      );
  ...
  describe('Utility Methods', () => {
    it('should check model availability', async () => {
      ...
      const notAvailable = await client.isModelAvailable(
        'definitely-not-a-real-model-xyz123',
      );
