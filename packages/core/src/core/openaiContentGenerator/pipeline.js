...
export class ContentGenerationPipeline {
    ...
    async execute(request, userPromptId) {
        ...
        return this.executeWithErrorHandling(request, userPromptId, false, effectiveModel, async (openaiRequest) => {
    ...
    async executeStream(request, userPromptId) {
        const effectiveModel = this.contentGeneratorConfig.model;
        this.converter.setModel(effectiveModel);
        return this.executeWithErrorHandling(request, userPromptId, true, effectiveModel, async (openaiRequest, context) => {
    ...
    async buildRequest(request, userPromptId, streaming = false, effectiveModel) {
     ...
     */
    async executeWithErrorHandling(request, userPromptId, isStreaming, effectiveModel, executor) {
        const context = this.createRequestContext(userPromptId, isStreaming, effectiveModel);
        try {
            const openaiRequest = await this.buildRequest(request, userPromptId, isStreaming, effectiveModel);
     ...
     */
    createRequestContext(userPromptId, isStreaming, effectiveModel) {
        return {
            userPromptId,
            model: effectiveModel,
