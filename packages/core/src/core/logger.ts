...
export class Logger {
  private ollamaCodeDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await fs.mkdir(this.ollamaCodeDir, { recursive: true });
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.ollamaCodeDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.ollamaCodeDir) {
      this.debugLogger.error(
