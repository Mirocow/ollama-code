...
async function getOllamaMdFilePathsInternal(currentWorkingDirectory, includeDirectoriesToReadOllama, userHomePath, fileService, extensionContextFilePaths = [], folderTrust) {
    const dirs = new Set([
    ...
    for (let i = 0; i < dirsArray.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map((dir) => getOllamaMdFilePathsInternalForEachDir(dir, userHomePath, fileService, extensionContextFilePaths, folderTrust));
...
async function getOllamaMdFilePathsInternalForEachDir(dir, userHomePath, fileService, extensionContextFilePaths = [], folderTrust) {
    const allPaths = new Set();
    const ollamaMdFilenames = getAllOllamaMdFilenames();
    for (const ollamaMdFilename of ollamaMdFilenames) {
        const resolvedHome = path.resolve(userHomePath);
        const globalMemoryPath = path.join(resolvedHome, OLLAMA_DIR, ollamaMdFilename);
        // This part that finds the global file always runs.
        try {
            await fs.access(globalMemoryPath, fsSync.constants.R_OK);
            allPaths.add(globalMemoryPath);
            logger.debug(`Found readable global ${ollamaMdFilename}: ${globalMemoryPath}`);
        }
        catch {
        ...
        if (isHomeDirectory) {
            ...
            try {
                ...
                if (homeContextPath !== globalMemoryPath) {
                    ...
                    logger.debug(`Found readable home ${ollamaMdFilename}: ${homeContextPath}`);
                }
            }
            catch {
            ...
            logger.debug(`Searching for ${ollamaMdFilename} starting from CWD: ${resolvedCwd}`);
    ...
    logger.debug(`Final ordered ${getAllOllamaMdFilenames()} paths to read: ${JSON.stringify(finalPaths)}`);
    return finalPaths;
}
async function readOllamaMdFiles(filePaths, importFormat = 'tree') {
    ...
    for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map(async (filePath) => {
            try {
                ...
                if (!isTestEnv) {
                    const message = error instanceof Error ? error.message : String(error);
                    logger.warn(`Warning: Could not read ${getAllOllamaMdFilenames()} file at ${filePath}. Error: ${message}`);
                }
 ...
 */
export async function loadServerHierarchicalMemory(currentWorkingDirectory, includeDirectoriesToReadOllama, fileService, extensionContextFilePaths = [], folderTrust, importFormat = 'tree') {
    ...
    const filePaths = await getOllamaMdFilePathsInternal(currentWorkingDirectory, includeDirectoriesToReadOllama, userHomePath, fileService, extensionContextFilePaths, folderTrust);
