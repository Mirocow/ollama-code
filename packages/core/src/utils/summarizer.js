 ...
 */
export const defaultSummarizer = (result, _ollamaClient, _abortSignal) => Promise.resolve(JSON.stringify(result.llmContent));
...
`;
export const llmSummarizer = (result, ollamaClient, abortSignal) => summarizeToolOutput(partToString(result.llmContent), ollamaClient, abortSignal);
export async function summarizeToolOutput(textToSummarize, ollamaClient, abortSignal, maxOutputTokens = 2000) {
    ...
    try {
        const parsedResponse = (await ollamaClient.generateContent(contents, toolOutputSummarizerConfig, abortSignal, DEFAULT_OLLAMA_MODEL));
