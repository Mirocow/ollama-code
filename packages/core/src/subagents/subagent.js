...
export class SubAgentScope {
    ...
    async runNonInteractive(context, externalSignal) {
        ...
        let currentRoundAbortController = null;
        ...
        try {
            ...
            while (true) {
                ...
                const responseStream = await chat.sendMessageStream(this.modelConfig.model ||
                    this.runtimeContext.getModel() ||
                    DEFAULT_OLLAMA_MODEL, messageParams, promptId);
                ...
                if (functionCalls.length > 0) {
                    currentMessages = await this.processFunctionCalls(functionCalls, roundAbortController, promptId, turnCounter, toolsList, currentResponseId);
                }
                else {
    ...
    async createChatObject(context) {
        ...
        try {
            ...
            return new OllamaChat(this.runtimeContext, generationConfig, start_history);
        }
        catch (error) {
