...
export class NativeLspService {
    ...
    constructor(config, workspaceContext, _eventEmitter, fileDiscoveryService, _ideContextStore, options = {}) {
        ...
        this.languageDetector = new LspLanguageDetector(this.workspaceContext, this.fileDiscoveryService);
    ...
    async discoverAndPrepare() {
        ...
        const extensionOverrides = this.configLoader.collectExtensionToLanguageOverrides([
            ...extensionConfigs,
            ...userConfigs,
        ]);
        const detectedLanguages = await this.languageDetector.detectLanguages(extensionOverrides);
        // Merge configs: built-in presets + extension LSP configs + user .lsp.json
        const serverConfigs = this.configLoader.mergeConfigs(detectedLanguages, extensionConfigs, userConfigs);
    ...
    async definitions(location, serverName, limit = 50) {
        ...
        for (const [name, handle] of handles) {
            try {
                await this.serverManager.warmupTypescriptServer(handle);
                const response = await handle.connection.request('textDocument/definition', {
    ...
    async references(location, serverName, includeDeclaration = false, limit = 200) {
        ...
        for (const [name, handle] of handles) {
            try {
                await this.serverManager.warmupTypescriptServer(handle);
                const response = await handle.connection.request('textDocument/references', {
                    textDocument: { uri: location.uri },
                    position: location.range.start,
    ...
    async implementations(location, serverName, limit = 50) {
        ...
        for (const [name, handle] of handles) {
            try {
                await this.serverManager.warmupTypescriptServer(handle);
                const response = await handle.connection.request('textDocument/implementation', {
                    textDocument: { uri: location.uri },
                    position: location.range.start,
                });
                const candidates = Array.isArray(response)
    ...
    async prepareCallHierarchy(location, serverName, limit = 50) {
        ...
        for (const [name, handle] of handles) {
            try {
                await this.serverManager.warmupTypescriptServer(handle);
                const response = await handle.connection.request('textDocument/prepareCallHierarchy', {
                    textDocument: { uri: location.uri },
                    position: location.range.start,
                });
                const candidates = Array.isArray(response)
