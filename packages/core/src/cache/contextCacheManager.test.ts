...
describe('ContextCacheManager', () => {
  ...
  describe('isContextCompatible', () => {
    it('should return true for same model', () => {
      cacheManager.setContext('session-1', [1, 2, 3], 'llama3.2', 1);
      expect(cacheManager.isContextCompatible('session-1', 'llama3.2')).toBe(
        true,
      );
    });
    ...
    it('should return false for different model', () => {
      cacheManager.setContext('session-1', [1, 2, 3], 'llama3.2', 1);
      expect(cacheManager.isContextCompatible('session-1', 'llama2')).toBe(
        false,
      );
    });

    it('should return false for non-existent session', () => {
      expect(cacheManager.isContextCompatible('non-existent', 'llama3.2')).toBe(
        false,
      );
    });
  });
  ...
  describe('handleGenerateResponse', () => {
    ...
    it('should handle response without context', () => {
      cacheManager.handleGenerateResponse('session-1', {}, 'llama3.2', 1);
  ...
  describe('Session state management', () => {
    ...
    it('should update lastActivity on session state update', async () => {
      cacheManager.initSession('session-1', 'llama3.2');
      const initialTime =
        cacheManager.getSessionState('session-1')?.lastActivity;
  ...
  describe('edge cases', () => {
    ...
    it('should handle model name with version', () => {
      ...
      expect(
        cacheManager.isContextCompatible('session-1', 'llama3.2:latest'),
      ).toBe(true);
      expect(cacheManager.isContextCompatible('session-1', 'llama3.2')).toBe(
        false,
      );
