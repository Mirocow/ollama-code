...
Инструмент для сохранения пользовательских фактов и предпочтений по явному запросу.

| Параметр | Описание |
|----------|----------|
| `fact` | Факт для запоминания (обязательно) |
...
```json
{
  "fact": "Я предпочитаю TypeScript для новых проектов",
  "scope": "global"
}
```
...
### Хранилище данных (`model_storage`)

**Каноническое имя**: `model_storage`  
**Алиасы**: `storage`, `store`, `kv`, `roadmap`

Универсальное key-value хранилище для внутренних нужд AI с поддержкой CRUD, TTL и метаданных.

| Параметр | Описание |
|----------|----------|
| `operation` | Операция: set/get/delete/list/append/merge/batch/stats |
| `namespace` | Пространство имён: roadmap/session/knowledge/context/learning/metrics |
| `key` | Ключ |
| `value` | Значение (любой JSON) |
| `ttl` | Время жизни в секундах |
| `tags` | Теги для категоризации |
| `scope` | `global` или `project` |

**Когда использовать:**
- Roadmap и планы проекта
- База знаний AI (паттерны, соглашения)
...
### Различия между `save_memory` и `model_storage`

| Характеристика | `save_memory` | `model_storage` |
|----------------|---------------|-----------------|
| **Инициатор** | По запросу пользователя | AI самостоятельно |
| **Формат** | Markdown | JSON |
| **Подтверждение** | Требуется | Автоматически |
| **Операции** | Только добавление | Полный CRUD |
| **TTL** | Нет | Да |
