...
Универсальное key-value хранилище для AI-модели для сохранения структурированных данных между сессиями.

**Параметры:**

| Имя         | Тип    | Обязательный | Описание                                              |
| ----------- | ------ | ------------ | ----------------------------------------------------- |
| `operation` | string | Да           | Операция для выполнения (см. ниже)                    |
| `namespace` | string | Да           | Пространство имён (roadmap, session, knowledge и др.) |
| `key`       | string | Нет*         | Ключ для сохранения/получения                         |
| `value`     | any    | Нет*         | Значение для сохранения (любой JSON-сериализуемый тип)|
| `scope`     | string | Нет          | `global` (все проекты) или `project` (текущий проект)|
...
| Операция | Описание                              |
| -------- | ------------------------------------- |
| `set`    | Сохранить значение (перезаписывает)   |
...
| Namespace  | Назначение                           | Персистентность |
| ---------- | ------------------------------------ | --------------- |
| `roadmap`  | Roadmap проекта, milestones, планы   | Постоянная      |
| `session`  | Временные данные сессии              | На сессию       |
| `knowledge`| Изученные факты, паттерны, предпочтения | Постоянная   |
| `context`  | Контекст текущей задачи и состояние  | На задачу       |
| `learning` | Алиасы инструментов, корректировки   | Постоянная      |
| `metrics`  | Статистика, данные производительности| Постоянная      |

**Примеры:**

```json
// Сохранить milestone roadmap
{
  "operation": "set",
  "namespace": "roadmap",
  "key": "v1.0",
...
// Добавить в backlog
{
  ...
  "namespace": "roadmap",
  "key": "backlog",
  "value": "Добавить тёмную тему"
}

// Сохранить изученный паттерн
{
  "operation": "merge",
  "namespace": "knowledge",
