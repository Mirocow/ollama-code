# Connect Ollama Code to tools via MCP

Ollama Code can connect to external tools and data sources through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/introduction). MCP servers give Ollama Code access to your tools, databases, and APIs.

## What you can do with MCP

With MCP servers connected, you can ask Ollama Code to:
...
Ollama Code loads MCP servers from `mcpServers` in your `settings.json`. You can configure servers either:
...
3. Restart Ollama Code in the same project (or start it if it wasn’t running yet), then ask the model to use tools from that server.

## Where configuration is stored (scopes)

Most users only need these two scopes:

- **Project scope (default)**: `.ollama-code/settings.json` in your project root
- **User scope**: `~/.ollama-code/settings.json` across all projects on your machine
...
JSON (`.ollama-code/settings.json`):
...
Use `includeTools` / `excludeTools` to restrict tools exposed by a server (from Ollama Code’s perspective).
...
- **Environment variables in JSON don’t resolve**: ensure they exist in the environment where Ollama Code runs (shell vs GUI app environments can differ).
