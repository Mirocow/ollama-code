...
Ollama Code offers three distinct permission modes that allow you to flexibly control how AI interacts with your code and system based on task complexity and risk level.
...
> [!tip]
>
> You can quickly cycle through modes during a session using **Shift+Tab** (or **Tab** on Windows). The terminal status bar shows your current mode, so you always know what permissions Ollama Code has.

## 1. Use Plan Mode for safe code analysis

Plan Mode instructs Ollama Code to create a plan by analyzing the codebase with **read-only** operations, perfect for exploring codebases, planning complex changes, or reviewing code safely.

### When to use Plan Mode

- **Multi-step implementation**: When your feature requires making edits to many files
- **Code exploration**: When you want to research the codebase thoroughly before changing anything
- **Interactive development**: When you want to iterate on the direction with Ollama Code
...
Ollama Code analyzes the current implementation and create a comprehensive plan. Refine with follow-ups:
...
Default Mode is the standard way to work with Ollama Code. In this mode, you maintain full control over all potentially risky operations - Ollama Code will ask for your approval before making any file changes or executing shell commands.
...
- **Learning and teaching**: When you want to understand each step Ollama Code is taking
...
Default Mode is the initial mode when you start Ollama Code. If you've changed modes and want to return to Default Mode, use:
...
Ollama Code will analyze your codebase and propose a plan. It will then ask for approval before:
...
Auto-Edit Mode instructs Ollama Code to automatically approve file edits while requiring manual approval for shell commands, ideal for accelerating development workflows while maintaining system safety.
...
1. You ask Ollama Code to refactor a function
...
YOLO Mode grants Ollama Code the highest permissions, automatically approving all tool calls including file editing and shell commands.
...
During a Ollama Code session, use **Shift+Tab**​ (or **Tab** on Windows) to quickly cycle through the three modes:
...
```
// Project-level: ./.ollama-code/settings.json
// User-level: ~/.ollama-code/settings.json
{
  "permissions": {
