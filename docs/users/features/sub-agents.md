...
Subagents are specialized AI assistants that handle specific types of tasks within Ollama Code. They allow you to delegate focused work to AI agents that are configured with task-specific prompts, tools, and behaviors.
...
- **Project-level**: `.ollama-code/agents/` (highest precedence)
- **User-level**: `~/.ollama-code/agents/` (fallback)
...
Ollama Code proactively delegates tasks based on:
