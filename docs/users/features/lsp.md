...
Ollama Code provides native Language Server Protocol (LSP) support, enabling advanced code intelligence features like go-to-definition, find references, diagnostics, and code actions. This integration allows the AI agent to understand your code more deeply and provide more accurate assistance.

## Overview

LSP support in Ollama Code works by connecting to language servers that understand your code. When you work with TypeScript, Python, Go, or other supported languages, Ollama Code can automatically start the appropriate language server and use it to:
...
LSP is an experimental feature in Ollama Code. To enable it, use the `--experimental-lsp` command line flag:

```bash
qwen --experimental-lsp
```

For most common languages, Ollama Code will automatically detect and start the appropriate language server if it's installed on your system.
...
Ollama Code exposes LSP functionality through the unified `lsp` tool. Here are the available operations:
...
5. **Verify --experimental-lsp flag**: Make sure you're using the flag when starting Ollama Code
...
Ollama Code supports Claude Code-style `.lsp.json` configuration files in the language-keyed format defined in the [Claude Code plugins reference](https://code.claude.com/docs/en/plugins-reference#lsp-servers). If you're migrating from Claude Code, use the language-as-key layout in your configuration.
...
Claude Code LSP plugins can also supply `lspServers` in `plugin.json` (or a referenced `.lsp.json`). Ollama Code loads those configs when the extension is enabled, and they must use the same language-keyed format.
...
Use the `--experimental-lsp` flag when starting Ollama Code:
