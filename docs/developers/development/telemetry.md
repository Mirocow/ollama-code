...
Learn how to enable and setup OpenTelemetry for Ollama Code.
...
observability framework — Ollama Code's observability system provides:
...
Logs are timestamped records of specific events. The following events are logged for Ollama Code:

- `ollama-code.config`: This event occurs once at startup with the CLI's configuration.
  - **Attributes**:
    - `model` (string)
...
- `ollama-code.user_prompt`: This event occurs when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `ollama-code.tool_call`: This event occurs for each function call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `ollama-code.file_operation`: This event occurs for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
- `ollama-code.api_request`: This event occurs when making a request to Qwen API.
  - **Attributes**:
    - `model`
    - `request_text` (if applicable)

- `ollama-code.api_error`: This event occurs if the API request fails.
  - **Attributes**:
    - `model`
    - `error`
    - `error_type`
...
- `ollama-code.api_response`: This event occurs upon receiving a response from Qwen API.
  - **Attributes**:
    - `model`
    - `status_code`
    - `duration_ms`
...
- `ollama-code.tool_output_truncated`: This event occurs when the output of a tool call is too large and gets truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `ollama-code.malformed_json_response`: This event occurs when a `generateJson` response from Qwen API cannot be parsed as a json.
  - **Attributes**:
    - `model`

- `ollama-code.flash_fallback`: This event occurs when Ollama Code switches to flash as fallback.
  - **Attributes**:
    - `auth_type`

- `ollama-code.slash_command`: This event occurs when a user executes a slash command.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, if applicable)

- `ollama-code.extension_enable`: This event occurs when an extension is enabled
- `ollama-code.extension_install`: This event occurs when an extension is installed
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
    - `extension_source` (string)
    - `status` (string)
- `ollama-code.extension_uninstall`: This event occurs when an extension is uninstalled

### Metrics

Metrics are numerical measurements of behavior over time. The following metrics are collected for Ollama Code (metric names remain `ollama-code.*` for compatibility):

- `ollama-code.session.count` (Counter, Int): Incremented once per CLI startup.

- `ollama-code.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `ollama-code.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
    - `decision` (string: "accept", "reject", or "modify", if applicable)

- `ollama-code.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `ollama-code.api.request.latency` (Histogram, ms): Measures API request latency.
  - **Attributes**:
    - `model`

- `ollama-code.token.usage` (Counter, Int): Counts the number of tokens used.
  - **Attributes**:
    - `model`
    - `type` (string: "input", "output", "thought", "cache", or "tool")

- `ollama-code.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
...
- `ollama-code.chat_compression` (Counter, Int): Counts chat compression operations
  - **Attributes**:
