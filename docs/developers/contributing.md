...
```bash
git clone https://github.com/QwenLM/ollama-code.git # Or your fork's URL
cd ollama-code
```
...
To start the Ollama Code application from the source code (after building), run the following command from the root directory:

```bash
npm start
```

If you'd like to run the source build outside of the ollama-code folder, you can utilize `npm link path/to/ollama-code/packages/cli` (see: [docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) to run with `ollama-code`
...
The integration tests are designed to validate the end-to-end functionality of Ollama Code. They are not run as part of the default `npm run test` command.
...
- `packages/`: Contains the individual sub-packages of the project.
  ...
  - `core/`: The core backend logic for Ollama Code.
...
**Note:** If you have `DEBUG=true` in a project's `.env` file, it won't affect ollama-code due to automatic exclusion. Use `.ollama-code/.env` files for ollama-code specific debug settings.

### React DevTools

To debug the CLI's React-based UI, you can use React DevTools. Ink, the library used for the CLI's interface, is compatible with React DevTools version 4.x.

1.  **Start the Ollama Code application in development mode:**
