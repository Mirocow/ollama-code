# MCP servers with Ollama Code

This document provides a guide to configuring and using Model Context Protocol (MCP) servers with Ollama Code.
...
Ollama Code integrates with MCP servers through a sophisticated discovery and execution system built into the core package (`packages/core/src/tools/`):
...
Ollama Code uses the `mcpServers` configuration in your `settings.json` file to locate and connect to MCP servers. This configuration supports multiple servers with different transport mechanisms.
...
Ollama Code supports OAuth 2.0 authentication for remote MCP servers using SSE or HTTP transports. This enables secure access to MCP servers that require authentication.
...
- **Stored securely** in `~/.ollama-code/mcp-oauth-tokens.json`
...
When Ollama Code starts, it performs MCP server discovery through the following detailed process:
...
When Ollama Code receives this response, it will:
...
In addition to tools, MCP servers can expose predefined prompts that can be executed as slash commands within Ollama Code. This allows you to create shortcuts for common or complex queries that can be easily invoked by name.
...
```bash
/poem-writer --title="Ollama Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Ollama Code" reverent
```
...
The `add` command configures a new MCP server in your `settings.json`. Based on the scope (`-s, --scope`), it will be added to either the user config `~/.ollama-code/settings.json` or the project config `.ollama-code/settings.json` file.
