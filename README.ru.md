...
## Новые возможности v0.17.11

### Авто-извлечение JSON Tool Calls из текста
...
Это делает систему более устойчивой для моделей, которые "сдаются" и выводят JSON как текст вместо реальных tool calls.

---

## Новые возможности v0.17.10

### Non-Interactive Mode и улучшения Storage Tool

**Исправления ошибок:**

| Проблема                          | Решение                                                     |
| --------------------------------- | ----------------------------------------------------------- |
| Двойной вывод "Loaded task file"  | Только в дочернем процессе (`OLLAMA_CODE_NO_RELAUNCH=true`) |
| "fetch failed" в addWithEmbedding | Сначала сохранить данные, эмбеддинг опционален              |
| Модель сдаётся после ошибки       | Добавлены подсказки в результатах инструментов              |

**Устойчивые операции Storage:**

```json
// До: Полный провал если Ollama не запущен
{ "operation": "addWithEmbedding", ... } // ❌ fetch failed

// После: Данные сохраняются даже без эмбеддингов
{ "operation": "addWithEmbedding", ... } // ✅ Data saved (embedding skipped)
```

**Подсказки модели в результатах инструментов:**

| Ситуация            | Добавленная подсказка                       |
| ------------------- | ------------------------------------------- |
| Успех               | "Continue with the next step in your task." |
| Эмбеддинг не удался | "Data saved successfully. Continue..."      |
| Ошибка              | "Try using 'set' operation instead..."      |

---

## Новые возможности v0.17.5

### Исправлено: Загрузка шаблонов и опция --file

**Исправления ошибок:**

| Проблема                              | Решение                                                     |
| ------------------------------------- | ----------------------------------------------------------- |
| Ошибка "Unknown file extension .md"   | Заменены статические импорты на runtime `fs.readFileSync()` |
| Неправильные пути к шаблонам в bundle | Добавлено определение путей для bundle/dist режимов         |
| Отсутствующие шаблоны                 | Fallback-шаблоны при отсутствии файлов                      |

**Новая опция --file:**

```bash
# Загрузить задачу по имени (из ~/.ollama-code/tasks/)
npm run cli -- --file tools-demo

# Загрузить с полным путём
npm run cli -- --file ~/my-tasks/project-audit.md

# Загрузить с относительным путём
...
## Новые возможности v0.17.2

### Инструмент Model Storage — Персистентная память AI
...
| `merge`  | Слить объект с существующим     |
| `clear`  | Очистить все данные в namespace |

#### Предопределённые namespaces

| Namespace   | Назначение                            | Режим по умолчанию |
| ----------- | ------------------------------------- | ------------------ |
| `roadmap`   | Дорожная карта, планы, milestones     | persistent         |
| `session`   | Временные данные сессии               | session            |
| `knowledge` | Изученные факты, паттерны, настройки  | persistent         |
| `context`   | Контекст текущей задачи               | session            |
| `learning`  | Алиасы инструментов, исправления      | persistent         |
| `metrics`   | Статистика, данные производительности | persistent         |

#### Области хранения

- **global**: `~/.ollama-code/storage/` (общие для всех проектов)
- **project**: `<project>/storage/` (данные конкретного проекта)
...
## Новые возможности v0.17.1

### Инструменты управления SSH профилями

Полная система управления SSH профилями для хранения и повторного использования учётных данных:

| Инструмент        | Описание                    |
| ----------------- | --------------------------- |
| `ssh_add_host`    | Сохранить SSH профиль       |
| `ssh_list_hosts`  | Список сохранённых профилей |
| `ssh_remove_host` | Удалить сохранённый профиль |

### Улучшенный SSH Connect
...
## Новые возможности v0.17.0

### Git Workflow Tool — Полная интеграция с Git
...
| `add`, `commit` | Индексация и коммит изменений    |
| `push`, `pull`  | Операции с remote                |
| `clone`         | Клонирование репозитория         |
| `create_branch` | Создание новой ветки             |
| `switch`        | Переключение на ветку            |
| `log`, `diff`   | История и различия               |
| `create_mr`     | Создание GitLab Merge Request    |
| `create_pr`     | Создание GitHub Pull Request     |
| `create_merge`  | Автоопределение и создание MR/PR |

### Операции аутентификации

| Операция      | Описание                                      |
| ------------- | --------------------------------------------- |
| `auth_status` | Проверка статуса аутентификации GitHub/GitLab |
| `auth_login`  | Интерактивные инструкции по входу             |
| `auth_logout` | Выход из GitHub/GitLab                        |
| `auth_token`  | Установка токена аутентификации               |

### Поддержка платформ

- **GitHub**: Полная поддержка через `gh` CLI
- **GitLab.com**: Полная поддержка через `glab` CLI
- **Self-hosted GitLab**: Автоопределение и поддержка

### SSH Tools Plugin

Новый плагин SSH Tools для подключения к удалённым серверам с управлением профилями:
