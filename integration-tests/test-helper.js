...
export class TestRig {
    ...
    setup(testName, options = {}) {
        ...
        const ollamaCodeDir = join(this.testDir, '.ollama-code');
        mkdirSync(ollamaCodeDir, { recursive: true });
        ...
        const settings = {
            ...
            sandbox: env.OLLAMA_SANDBOX !== 'false' ? env.OLLAMA_SANDBOX : false,
            ...options.settings, // Allow tests to override/add settings
        };
        writeFileSync(join(ollamaCodeDir, 'settings.json'), JSON.stringify(settings, null, 2));
    }
    ...
    /**
     * The command and args to use to invoke Ollama Code CLI. Allows us to switch
     * between using the bundled cli.js (the default) and using the installed
     * 'ollama-code' (used to verify npm bundles).
     */
    _getCommandAndArgs(extraInitialArgs = []) {
        const isNpmReleaseTest = process.env.INTEGRATION_TEST_USE_INSTALLED_OLLAMA_CODE === 'true';
        const command = isNpmReleaseTest ? 'ollama-code' : 'node';
    ...
    async waitForTelemetryEvent(eventName, timeout) {
        ...
        return this.poll(() => {
            ...
            return logs.some((logData) => logData.attributes &&
                logData.attributes['event.name'] === `ollama-code.${eventName}`);
        }, timeout, 100);
    }
                    ...
                    if (braceDepth === 0) {
                        ...
                        try {
                            ...
                            if (obj.body &&
                                ...
                                obj.attributes['event.name'] === 'ollama-code.tool_call') {
                                logs.push({
    ...
    readToolLogs() {
        ...
        for (const logData of parsedLogs) {
            ...
            if (logData.attributes &&
                logData.attributes['event.name'] === 'ollama-code.tool_call') {
    ...
    readLastApiRequest() {
        ...
        const apiRequests = logs.filter((logData) => logData.attributes &&
            logData.attributes['event.name'] === 'ollama-code.api_request');
            ...
            if (logData.scopeMetrics) {
                for (const scopeMetric of logData.scopeMetrics) {
                    for (const metric of scopeMetric.metrics) {
                        if (metric.descriptor.name === `ollama-code.${metricName}`) {
                            return metric;
                        }
                    }
                }
